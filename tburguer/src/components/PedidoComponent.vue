    ...
    <div>
      <form id="pedido-form" @submit="criarPedido($event)">
        ...
        <div class="inputs" id="form-pedido">
          ...
          <input type="text" id="nome-cliente" name="nome-cliente" v-model="nomeCliente" placeholder="Digite seu nome" />
        </div>
        <div class="inputs">
        ...
        <div id="opcionais-title" class="inputs">
          ...
          <label id="opcionais-subtitle" for="Complemento">Adicione um complemento</label>

          <div class="checkbox-container" v-for="complemento in listaComplementos" :key="complemento.id">
            <input type="checkbox" :name="complemento.nome" v-model="listaComplementoSelecionado" :value="complemento" />
            <span>{{ complemento.nome }}</span>
          </div>
          <label id="opcionais-subtitle" for="Complemento">Adicione a bebdida</label>

          <div class="checkbox-container" v-for="bebida in listaBebidas" :key="bebida.id">
            <input type="checkbox" :name="bebida.nome" v-model="listaBebidasSelecionadas" :value="bebida" />
...
export default {
  ...
  methods: {
    ...
    async criarPedido(e) {
      ...
      //se estiver vazio
      if (!dadosPedido.ponto) {
        this.msg = "O campo Ponto deve ser preenchido !"
        return;
      } else if (!dadosPedido.nome) {
        this.msg = "O campo Nome deve ser preenchido !"
        return;
      }
...
#foto-content {
  ...
  border-radius: 16px;
  position: relative;
  /* Adicionado: definindo posição relativa */
  z-index: -1;
